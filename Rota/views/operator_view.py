...
class OperatorView(QWidget):
    ...
    def setup_ui(self):
        ...
        # --- İSTASYON SEÇİCİ (YENİ) ---
        # Kullanıcıya tanımlı istasyonu al, yoksa varsayılan KESİM yap
        ...
        if user_station and user_station in ["KESİM", "RODAJ", "DELİK", "TEMPER", "LAMİNE", "ISICAM", "KUMLAMA", "SEVKİYAT"]:
            ...
            # Eğer Admin değilse değiştiremesin (Opsiyonel, şimdilik açık bırakıyorum kolaylık olsun)
            # if self.user.get('role') != 'admin': self.combo_station.setEnabled(False)
    ...
    def report_breakage(self):
        ...
        if ok:
            db.report_fire(self.current_order['id'], qty)
            QMessageBox.critical(self, "FİRE KAYDEDİLDİ", f"{qty} adet cam fire olarak işlendi.")
            self.refresh_list()

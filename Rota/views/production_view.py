...
Ozellikler:
- Karar Destek oncelik siralamasini kullanir (db'den priority)
- Istasyonlar rota sirasina gore gosterilir
- Sevkiyat istasyonu gosterilmez (ayri modul)
- Teslim tarihi ve gecikme uyarisi
"""
...
class FactoryConfig:
    ...
    STATION_ORDER = [
        ...
        "LAMINE A1", "ISICAM B1"
        # SEVKIYAT DAHIL DEGIL - ayri modul
    ]
    
    # Gosterilmeyecek istasyonlar
    HIDDEN_STATIONS = ["SEVKIYAT", "SEVKİYAT"]
    
    @classmethod
    def get_station_index(cls, station_name):
        """Istasyonun sira indeksini dondur"""
        try:
...
class ProductionView(QWidget):
    ...
    def _create_list_panel(self):
        ...
        lbl = QLabel("Siparisler (Karar Destek Sirasina Gore)")
    ...
    def refresh_data(self):
        """Verileri yenile - Karar Destek sira numarasini (queue_position) kullanir"""
        try:
            if db:
                # Uretim matrisini al
                matrix_data = db.get_production_matrix_advanced()
                
                # Orders tablosundan queue_position, priority ve delivery_date al
                orders_info = {}
                try:
                    with db.get_connection() as conn:
                        # queue_position kolonu var mi kontrol et
                        try:
                ...
                # Karar Destek siralamasina gore sirala (queue_position)
                matrix_data.sort(key=lambda x: (x.get('queue_position', 9999), x.get('delivery_date', '9999-12-31')))
                
                self.all_orders = matrix_data
            else:
                self.all_orders = []
            
            self.update_list()
            self.update_stats()
            self.status_label.setText(f"{len(self.all_orders)} siparis | {datetime.now().strftime('%H:%M:%S')}")
        except Exception as e:
    ...
    def _sort_stations_by_route(self, status_map, route_str):
        """Istasyonlari rota sirasina gore sirala"""
        if route_str:
            # Rotadaki sirayi kullan
            ...
            # Varsayilan fabrika sirasini kullan
    ...
    def open_production_dialog(self, station_name, station_info):
        ...
        if dialog.exec() == QDialog.Accepted and dialog.result_qty > 0:
            try:
                order_id = self.selected_order.get('id')
                if db and order_id:
                    db.register_production(order_id, station_name, dialog.result_qty)
                    
                    self.status_label.setText(f"{dialog.result_qty} adet {station_name} kaydedildi")
                    
                    self.refresh_data()
                    
                    # Ayni siparisi tekrar goster
                    for order in self.all_orders:
                        if order.get('id') == order_id:
                            self.show_detail(order)
                            break
            except Exception as e:
    ...
    def report_fire(self):
        ...
        if ok and db:
            try:
                db.report_fire(self.selected_order.get('id'), qty)
                self.status_label.setText(f"{qty} adet fire kaydedildi")
                self.refresh_data()
            except Exception as e:
